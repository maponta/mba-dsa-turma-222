...
reviews_words_lem <- read_xlsx("./Datasets/2023-total-volume-20000.xlsx")
reviews_words_lem$id <- seq.int(nrow(reviews_words_lem))
reviews_words_lem <- reviews_words_lem %>% select(id, company, store, content, at)
reviews_words_lem <- reviews_words_lem %>%
  mutate(store = tolower(store))
reviews_words_lem$at <- as.POSIXct(reviews_words_lem$at, format = "%Y-%m-%d %H:%M:%S")
reviews_words_lem <- subset(reviews_words_lem, as.Date(at) != as.Date("2023-08-07"))
...
for (i in 1:nrow(combinacoes)) {
  ...
  fileBow <- paste0("Output/normal_bow_n2_itau_not_07082023/", output, "-bow-n2-normal-wordcloud.png")
  fileBar <- paste0("Output/normal_bow_n2_itau_not_07082023/", output, "-bow-n2-normal-bar.png")
  ...
  graph <- ggplot(graph_data, aes(x = n, y = forcats::fct_rev(forcats::fct_infreq(word)))) +
    ...
    labs(title = "Bag of words n2 normal", x = "Frequência", y = "Palavra")
