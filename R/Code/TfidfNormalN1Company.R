...
fileBow <- paste0("Output/normal_tfidf_n1_company/", "all", "-bow-n1-wordcloud.png")
fileBar <- paste0("Output/normal_tfidf_n1_company/", "all", "-bow-n1-bar.png")
...
names(df) <- c("word")
df$company <- reviews_words_by_company$company
df <- df %>% filter(word != "")
