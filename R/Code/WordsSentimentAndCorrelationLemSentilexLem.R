...
for (i in 1:nrow(combinacoes)) {
  ...
  fileWordcloudPositive <- paste0("Output/lem_sentiment_correlation_sentiLex_lem/", output, "-postive-wordcloud.png")
  fileWordcloudNeutral <- paste0("Output/lem_sentiment_correlation_sentiLex_lem/", output, "-neutral-wordcloud.png")
  fileWordcloudNegative <- paste0("Output/lem_sentiment_correlation_sentiLex_lem/", output, "-negative-wordcloud.png")
  fileBarPositive <- paste0("Output/lem_sentiment_correlation_sentiLex_lem/", output, "-postive-graph-bar.png")
  fileBarNeutral <- paste0("Output/lem_sentiment_correlation_sentiLex_lem/", output, "-neutral-graph-bar.png")
  fileBarNegative <- paste0("Output/lem_sentiment_correlation_sentiLex_lem/", output, "-negative-graph-bar.png")
  fileTopPositive <- paste0("Output/lem_sentiment_correlation_sentiLex_lem/", output, "-positive-top-correlation.png")
  fileTopNegative <- paste0("Output/lem_sentiment_correlation_sentiLex_lem/", output, "-negative-top-correlation.png")
  fileTopNeutral <- paste0("Output/lem_sentiment_correlation_sentiLex_lem/", output, "-neutral-top-correlation.png")
  ...
  corpus = tm_map(corpus, removeWords, c('real'))
  ...
  png(fileWordcloudPositive, width = 1200, height = 1200, res = 200)
  positive_wordcloud <- text_sentiment_positive %>% 
    with(wordcloud(word, n, random.order = FALSE, max.words = 50, colors="blue"))
  dev.off()
  
  png(fileWordcloudNeutral, width = 1200, height = 1200, res = 200)
  neutral_wordcloud <- text_sentiment_neutral %>% 
    with(wordcloud(word, n, random.order = FALSE, max.words = 50, colors="blue"))
  dev.off()
  
  png(fileWordcloudNegative, width = 1200, height = 1200, res = 200)
  negative_wordcloud <- text_sentiment_negative %>% 
    with(wordcloud(word, n, random.order = FALSE, max.words = 50, colors="blue"))
  dev.off()
  
  positive_graph_data <- head(text_sentiment_positive, 15)
  ...
  topPositive <- head(text_sentiment_positive, 4)$word
  topNeutral <- head(text_sentiment_neutral, 4)$word
  topNegative <- head(text_sentiment_negative, 4)$word
  
  wcplot_positive <- word_cors %>%
    filter(item1 %in% topPositive) %>%
    group_by(item1) %>%
  ...
  print(paste('correlation positive generated', output))
  
  wcplot_negative <- word_cors %>%
    filter(item1 %in% topNegative) %>%
    group_by(item1) %>%
    slice_max(correlation, n = 6) %>%
    ungroup() %>%
    mutate(item2 = reorder(item2, correlation)) %>%
  ...
  print(paste('correlation negative generated', output))
  
  wcplot_neutral <- word_cors %>%
    filter(item1 %in% topNeutral) %>%
    group_by(item1) %>%
    slice_max(correlation, n = 6) %>%
    ungroup() %>%
    mutate(item2 = reorder(item2, correlation)) %>%
    ...
    geom_bar(stat = "identity") +
    facet_wrap(~ item1, scales = "free") +
    labs(title = paste0(output, ' Neutral'), x = "Palavra", y = "Frequência") +
    coord_flip()
  
  print(paste('correlation neutral generated', output))
  
  ggsave(fileTopPositive, plot = wcplot_positive, width = 10, height = 6, units = "in")
  ggsave(fileTopNegative, plot = wcplot_negative, width = 10, height = 6, units = "in")
  ggsave(fileTopNeutral, plot = wcplot_neutral, width = 10, height = 6, units = "in")
